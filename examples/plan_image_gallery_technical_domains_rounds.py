...
SKEPTIC_MEETING_BRIEF = """\
Audit the discussion across all technical domains rather than only attacking isolated
numeric thresholds. Challenge unsupported capabilities, small-sample generalizations,
redundant components, hidden incompatibilities, missing technology families, false
ground-truth claims, filter-first assumptions, composite-score assumptions, API
optimism, clustering optimism, safety claims, and complexity without measurable value.

You may propose falsification tests or simpler alternatives. Do not reject an idea
only because it is unfamiliar or not currently installed. End every round with an
explicit `VERDICT: ACCEPT`, `VERDICT: REVISE`, or `VERDICT: REJECT`, where ACCEPT means
the discussion is sufficiently supported for Planner synthesis, not that every
participant's preferred technique must be adopted.
"""
...
def main() -> None:
    ...
    participants = [
        ...
        ParticipantConfig(
            ...
            meeting_brief=SKEPTIC_MEETING_BRIEF,
            model="deepseek-v4-pro",
            provider="deepseek",
            reasoning_effort="high",
            max_iterations=12,
