"""Same pipeline as plan_image_gallery_technical_domains_auto_roster.py, with two
additions layered on top:

1. A pre-roster exploration/confirmation phase. Before
   agent_meeting.role_architect.design_domain_roster() ever runs, an exploration
   agent (agent_meeting.exploration.run_exploration_phase) reads the task spec and
   is free to stop and ask the human stakeholder concrete clarifying questions --
   rendered as a Claude-Code-style numbered-menu-plus-"Other" terminal prompt
   (agent_meeting.interactive.ask_choice) -- about any real ambiguity it finds:
   an unstated priority, an implicit scope boundary, a tradeoff the spec gestures
   at without picking a side. Only once that's done (or the agent decides nothing
   more needs asking) does the roster get designed, now from a task spec with the
   confirmed answers folded in -- so the domains and briefs are shaped by what the
   human actually meant, not a guess resolved silently downstream.

2. A judge that can actually talk to you, every round. mode="planning_rounds" already
   runs a judge after each discussion round to decide stop/continue; this run sets
   MeetingConfig.human_checkin=True, which swaps the judge from a single deterministic
   LLM call (judge.py's judge_should_stop) for a small GeneralAgent tool loop
   (judge.py's run_interactive_judge) with exactly two tools: ask_user_question and
   submit_judgment (judge_tools.py). The judge can call ask_user_question any number
   of times, back and forth -- if your answer shows you didn't understand its
   question, or is itself a question back to it, it clarifies and asks again rather
   than guessing -- and only calls submit_judgment once it's actually gotten what it
   needed (including zero questions, if nothing that round genuinely warranted one).
   Every exchange is folded into that round's transcript as its own explicit "Human
   (AUTHORITATIVE)" turn, clearly marked as authoritative human input rather than one
   more position to argue with, so every later round (and the Planner) sees it exactly
   like any other round contribution, just not one they get to revise.

Everything else -- MEETING_INSTRUCTIONS, TECHNOLOGY_LANDSCAPE, PARTICIPANT_TASK_SPEC,
the VisualAuditor participant, the Planner, the CLI flags, the roster
save/load-for-resume convention -- is identical to
plan_image_gallery_technical_domains_auto_roster.py. See that file's docstring for the
full list of differences from plan_image_gallery_pipeline_planning_rounds.py further
upstream.

Run:
    C:\\Users\\LX034\\miniconda3\\python.exe examples\\plan_image_gallery_technical_domains_explore_confirm.py

Resume:
    C:\\Users\\LX034\\miniconda3\\python.exe examples\\plan_image_gallery_technical_domains_explore_confirm.py --resume mtg_xxxxxxxxxx

Skip any unfinished discussion round and synthesize immediately from the last
fully-checkpointed round:
    C:\\Users\\LX034\\miniconda3\\python.exe examples\\plan_image_gallery_technical_domains_explore_confirm.py --resume mtg_xxxxxxxxxx --planner-only

Reopen a completed meeting for more discussion:
    C:\\Users\\LX034\\miniconda3\\python.exe examples\\plan_image_gallery_technical_domains_explore_confirm.py --resume mtg_xxxxxxxxxx --extra-rounds 2

Note on --resume: the exploration phase and the role-architect roster both only ever
run once, on a fresh (non-resumed) start -- both their outputs are written to disk
before the meeting proper begins (runs/<meeting_id>_exploration_qa.json and
runs/<meeting_id>_domain_roster.json) so a --resume/--extra-rounds/--planner-only
reloads the same confirmed answers and the same participants instead of re-running an
interactive phase or calling the (LLM-backed) role architect again, either of which
could return something different from what the transcript on disk already reflects.
Per-round human_checkin answers, unlike those two, are not something to "resume" --
they're saved as part of each round's own checkpointed step the moment they're given,
same as everything else in that round.
"""
...
from agent_meeting.exploration import ExplorationResult, run_exploration_phase
...
def _exploration_path(meeting_id: str) -> Path:
    return meeting_path(meeting_id).with_name(f"{meeting_id}_exploration_qa.json")


def _save_exploration(meeting_id: str, result: ExplorationResult) -> None:
    payload = {"qa": result.qa, "summary": result.summary}
    _exploration_path(meeting_id).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")


def _load_exploration(meeting_id: str) -> ExplorationResult:
    path = _exploration_path(meeting_id)
    if not path.exists():
        # Older meetings, or a meeting_id that predates this script, simply never ran
        # an exploration phase -- treat that as "nothing was asked" rather than
        # refusing to resume, since the roster file (checked separately by
        # _load_roster) is the thing that's actually load-bearing for --resume.
        return ExplorationResult()
    payload = json.loads(path.read_text(encoding="utf-8"))
    return ExplorationResult(qa=payload.get("qa") or [], summary=payload.get("summary") or "")


# Loaded from disk (rather than inlined as a string literal, as this used to be) so
# the task spec fed to the exploration phase, the role architect, and every
# participant matches whatever the repo's own task-definition file currently says,
# with no risk of the two drifting apart the way a copy-pasted duplicate eventually
# would.
...
This meeting also runs a human check-in: after every round, the judge asks the human
stakeholder one concrete question and records their answer directly in the transcript
as a turn from "Human (AUTHORITATIVE)". Treat that turn as a hard constraint the same
way you would treat an explicit line in the task spec -- it is the actual task owner's
answer, not a participant's opinion, and it is not open for you to argue against or
revise; incorporate it into your position instead.
"""
...
- Treat any turn from "Human (AUTHORITATIVE)" as a directive from the actual task
  owner, not one more discussion contribution to weigh against participants'
  arguments -- when a human answer conflicts with what participants converged on
  through discussion, the human answer wins, and the Plan should say so plainly
  (state what the human decided and why the Plan follows it) rather than silently
  picking a side or trying to reconcile the two.
...
- Do not name participants, round numbers, or the meeting process itself anywhere
  ...
  whole thing, not as a summary of who said what. The one exception is the human
  stakeholder's own answers -- state what was decided and why, without needing to
  say "the human said" or reference the check-in mechanism itself.
...
def main() -> None:
    ...
    if resume:
        ...
        exploration = _load_exploration(meeting_id)
        print(f"[role-architect] reusing the {len(roster)}-domain roster saved for {meeting_id}:")
        for role in roster:
            print(f"  - {role.name}")
        if exploration.qa:
            print(f"[exploration] reusing {len(exploration.qa)} saved clarifying question(s)")
    else:
        meeting_id = new_meeting_id()

        print("[exploration] pre-roster exploration phase -- may ask clarifying questions...")
        exploration = run_exploration_phase(
            task_spec=PARTICIPANT_TASK_SPEC,
            meeting_id=meeting_id,
        )
        # Written before design_domain_roster() runs, same reasoning as the roster
        # file below: a crash after this point should not lose confirmed answers a
        # human already gave.
        _save_exploration(meeting_id, exploration)
        if exploration.qa:
            print(f"[exploration] {len(exploration.qa)} question(s) confirmed:")
            for entry in exploration.qa:
                print(f"  Q: {entry['question']}")
                print(f"  A: {entry['answer']}")
        else:
            print("[exploration] no clarifying questions were needed")

        task_spec_for_roster = PARTICIPANT_TASK_SPEC
        addendum = exploration.render_task_spec_addendum()
        if addendum:
            task_spec_for_roster = f"{PARTICIPANT_TASK_SPEC}\n\n{addendum}"

        print("[role-architect] designing the technology-domain roster from the task spec + evidence...")
        roster = design_domain_roster(
            task_spec=task_spec_for_roster,
            meeting_id=meeting_id,
    ...
    exploration_addendum = exploration.render_task_spec_addendum()
    if exploration_addendum:
        # Also folded into the meeting question itself (not just the roster's task
        # spec) so every participant -- not just the role architect that used it to
        # shape the roster -- sees the confirmed human answers from round 1 onward.
    ...
    for step in result["steps"]:
        ...
        for turn in step["turns"]:
            if turn.get("decided_by") == "human_checkin":
                print(
                    f"\n{'=' * 20} {turn['agent']} -- round {turn['round']} "
                    f"{'=' * 20}\n{turn['output']}"
                )
                continue
